...
from mvn.models.loss import Keypoints2dSmoothL1Loss, KeypointsMSELoss, KeypointsMSESmoothLoss, KeypointsMAELoss, \
    KeypointsL2Loss, VolumetricCELoss

from mvn.utils import img, multiview, op, vis, misc, cfg
from mvn.datasets import human36m, stb
...
def setup_stb_dataloaders(config, is_train, distributed_train):
    train_dataloader = None
    if is_train:
        # train
        train_dataset = stb.STBMultiViewDataset(
            stb_root=config.dataset.train.stb_root,
            pred_results_path=config.dataset.train.pred_results_path if hasattr(config.dataset.train, "pred_results_path") else None,
            train=True,
            test=False,
            image_shape=config.image_shape if hasattr(config, "image_shape") else (256, 256),
    ...
    # val
    val_dataset = stb.STBMultiViewDataset(
        stb_root=config.dataset.val.stb_root,
        pred_results_path=config.dataset.val.pred_results_path if hasattr(config.dataset.val, "pred_results_path") else None,
        train=False,
        test=True,
        image_shape=config.image_shape if hasattr(config, "image_shape") else (256, 256),
...
def setup_dataloaders(config, is_train=True, distributed_train=False):
    ...
    elif config.dataset.kind == 'stb':
        train_dataloader, val_dataloader, train_sampler = setup_stb_dataloaders(config, is_train, distributed_train)
    else:
    ...
    with grad_context():
        ...
        for iter_i, batch in iterator:
            ...
            if config.opt.loss_2d:
                k3d_gt_h = torch.cat(
                    [keypoints_3d_gt,
                     torch.ones(keypoints_3d_gt.shape[:2]+(1,), dtype=torch.float32, device=keypoints_3d_gt.device)],
                    dim=-1)
                k3d_gt_h = torch.transpose(k3d_gt_h, -2, -1)
                k2d_gt_h = proj_matricies_batch @ k3d_gt_h[:, None, :, :]
                k2d_gt_h = torch.transpose(k2d_gt_h, -2, -1)
                k2d_gt_h = k2d_gt_h / k2d_gt_h[..., -1:]
                k2d_gt = k2d_gt_h[..., :-1]
                loss = criterion(keypoints_2d_pred, k2d_gt)
            else:
            ...
            if base_points_pred is not None:
                ...
                for batch_i in range(batch_size):
                    ...
                    elif config.model.kind == "stb":
                        base_point_gt = torch.mean(keypoints_3d_gt[batch_i, :, :3], dim=0)
...
def main(args):
    ...
    # criterion
    criterion_class = {
        "SmoothL12D": Keypoints2dSmoothL1Loss,
